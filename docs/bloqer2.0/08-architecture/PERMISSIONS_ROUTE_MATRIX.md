...
- **Cronograma — `/proyectos/[id]/cronograma`:** `getProjectScheduleWorkspace` + Server Actions en `apps/web/features/schedule/actions/`; gate tenant **`SCHEDULE`**; `canViewScheduleArea` / `canEditScheduleArea` (`packages/services/src/schedule/schedule-access.ts`). Línea base presupuesto = misma resolución que control de costos (`APPROVED` / `CLOSED`). Export PDF/XLSX: `GET /api/reports/proyectos/[projectId]/cronograma?format=pdf|xlsx` (mismos filtros `budgetId` / `status` / `type` / `delayedOnly`).
