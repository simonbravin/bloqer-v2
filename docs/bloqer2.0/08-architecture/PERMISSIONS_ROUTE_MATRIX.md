...
| `/finanzas/gastos-generales`, `/finanzas/gastos-generales/nueva` | Same | **Imputación GG** (`VIEW AP`); cierre de período AUTO (`EDIT AP`). **`/nueva`:** redirect a `/finanzas/facturas-proveedor/nueva`. Alta corporativa: `facturas-proveedor` o `transacciones?register=ap`. |
| `/finanzas/cuentas-por-pagar`, `/finanzas/cuentas-por-pagar/[payableId]`, `/finanzas/cuentas-por-pagar/[payableId]/pagar` | Same | Aging corporativo (`corporateOnly`) + `listCompanyPayables`; `/finanzas/cuentas-por-pagar-aging` redirige aquí. |
| `/finanzas/cuentas-por-cobrar`, `/finanzas/cuentas-por-cobrar-aging` (redirect) | Same | Aging AR tenant + `listCompanyReceivables`; detalle/cobro en proyecto. |
