...
| Política de compras (`company-procurement-settings`) | `TENANT_SETTINGS` (o OWNER/ADMIN) | — |
| Overview proyecto — contador solicitudes (`getProjectOverviewDashboard`) | `canViewPurchaseRequests` | `PROCUREMENT` |
...
| Route | Access | Notes |
|-------|--------|-------|
| `/finanzas` | Authenticated; `buildTenantServiceContext` no nulo | **Layout** `app/(app)/finanzas/layout.tsx`: **`getFinanceSubnavLinks(ctx)`** — Resumen, Transacciones, Facturas y gastos, CxC, CxP, Imputación GG, Tesorería (según módulo + `can()`). Sidebar global incluye Facturas y gastos. **Resumen:** **`getFinanceHubOverview(ctx)`**. Sin rutas `-aging`. |
| `/finanzas/facturas-proveedor`, `/finanzas/facturas-proveedor/[invoiceId]` | Authenticated; tenant module **AP** | Servicios: `listCompanySupplierInvoices`, `getCompanySupplierInvoiceById`, `createSupplierInvoice` / `issue` / `cancel` (sin `projectScopeId`). **Lecturas y listados corporativos: solo `VIEW AP`** (`canViewCompanyAp` en servicios) — **no** alcanza solo `VIEW PROJECTS`. Alta corporativa en diálogo sobre el listado; mutaciones alta/emisión/anulación: `EDIT AP`. Adjuntos: `EntityDocumentsPanel` alcance `company-finanzas` + `listEntityDocuments` sin `projectId`. |
| `/finanzas/gastos-generales` | Same | **Imputación GG** (`VIEW AP`); cierre de período AUTO (`EDIT AP`). Alta corporativa: diálogo en `/finanzas/facturas-proveedor` o `transacciones?register=ap`. |
| `/finanzas/cuentas-por-pagar`, `/finanzas/cuentas-por-pagar/[payableId]`, `/finanzas/cuentas-por-pagar/[payableId]/pagar` | Same | Aging corporativo (`corporateOnly`) + `listCompanyPayables`. |
...
| `PURCHASE_REQUEST` / `PROCUREMENT_QUOTE` | `EDIT PURCHASE_REQUESTS` **or** `EDIT PROCUREMENT` (quotes: `canManageProcurementQuotes`) | `VIEW PURCHASE_REQUESTS` **or** `VIEW PROCUREMENT` |
...
| Purchase request — list / get | `VIEW PURCHASE_REQUESTS` **or** `canViewPurchaseRequests` | `canViewPurchaseRequests` = `VIEW PURCHASE_REQUESTS` **or** `VIEW PROCUREMENT` **or** `VIEW PROJECTS` |
| Purchase request — create / submit / cancel | `canEditPurchaseRequests` | `EDIT PURCHASE_REQUESTS` **or** `EDIT PROCUREMENT` |
...
- **Fase 11E+ — lock sourced + cola [D-063]:** `updateJournalEntry` bloquea montos/estructura en DRAFT con origen; notif `ACCOUNTING_DRAFTS_PENDING` a `EDIT ACCOUNTING` (dedupe 24h, soft). Contabilizar sigue exigiendo `EDIT ACCOUNTING`.
...
- **Roles:** `OWNER`, `ADMIN`, `FINANCE`, `TREASURER`, `PROJECT_FINANCE`, `PROCUREMENT`, `WAREHOUSE`, `SALES`, `VIEWER`, `PROJECT_MANAGER`, `SITE_FOREMAN`, `PROJECT_VIEWER` — todos definidos en `packages/domain/src/permissions/roles.ts` y presentes en `MATRIX` (`matrix.ts`). La UI de matriz de permisos (`matrix-overview` + `/configuracion/permisos`) usa `OVERVIEW_ROLES` (incluye `TREASURER` y `PROJECT_FINANCE`). **D-056:** company finance hub/tesorería/GL solo `OWNER|ADMIN|FINANCE|TREASURER|VIEWER` vía `hasCompanyFinanceRole` + helpers `canViewCompany*`.
- **Top-level nav** (`global-workspace-nav.ts` + `satisfiesNavRequirement` en `nav-config.ts`): cada ítem con `require` mapea a un `can(VIEW, …)` real **y** (Phase **12B**) disponibilidad del módulo para el tenant (`getTenantModuleGate`); **Inicio** sin `require` (cualquier usuario con shell de app). Módulos operativos bajo **proyecto** (certificaciones, compras, subcontratos, facturas, etc.) **no** tienen ítem de primer nivel; el acceso pasa por `VIEW PROJECTS` y gates por servicio — **12C** recomienda extender el gate a esos servicios.
...
- **UI app:** `(app)/layout.tsx` serializa `moduleGateSnapshot` y lo entrega al shell; `sidebar.tsx` reconstruye el gate y llama `buildGlobalNavSections(roles, (module) => gate.isEnabled(module))`. **Inicio** y enlace **Plataforma** no usan flags de tenant.
...
- **Phase 15B — resumen `/proyectos/[id]`:** `getProjectOverviewDashboard` (`packages/services/src/project/project-overview-dashboard.service.ts`) — KPIs y conteos con mismos gates/permisos que las vistas hijas; `getProjectFinanceOverview(ctx, projectId, { gate })` para no duplicar `getTenantModuleGate`; ficha extendida y acciones de ciclo de vida en página solo si `VIEW PROJECTS` (`getProjectById`). Contador **solicitudes de compra** (`purchaseRequestsCount`) con `canViewPurchaseRequests`; alerta si hay `SUBMITTED` y el usuario tiene `canEditPurchaseOrders` → enlace `/solicitudes-compra?status=SUBMITTED`.
- **Cronograma — `/proyectos/[id]/cronograma`:** `getProjectScheduleWorkspace` + Server Actions en `apps/web/features/schedule/actions/`; gate tenant **`SCHEDULE`**; `canViewScheduleArea` / `canEditScheduleArea` (`packages/services/src/schedule/schedule-access.ts`). Línea base presupuesto = misma resolución que control de costos (`APPROVED` / `CLOSED`). Export PDF/XLSX: `GET /api/reports/proyectos/[projectId]/cronograma?format=pdf|xlsx` (filtros `budgetId` / `status` / `type` / `delayedOnly`; contenido `view=gantt|table|both`; lapso opcional `from` / `to` ISO `YYYY-MM-DD`).
- **PM vs aging global:** **Cerrado [D-056].** Aging / hub company requieren `canViewCompanyAr` / `canViewCompanyAp` (rol company-finance). PM y `PROJECT_FINANCE` usan rutas de proyecto.
