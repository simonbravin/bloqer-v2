...
- **Roles:** `OWNER`, `ADMIN`, `FINANCE`, `PROJECT_FINANCE`, `PROCUREMENT`, `WAREHOUSE`, `SALES`, `VIEWER`, `PROJECT_MANAGER`, `SITE_FOREMAN`, `PROJECT_VIEWER` — todos definidos en `packages/domain/src/permissions/roles.ts` y presentes en `MATRIX` (`matrix.ts`). La UI de matriz de permisos (`matrix-overview` + `/configuracion/permisos`) usa `OVERVIEW_ROLES` (incluye `PROJECT_FINANCE`). **D-056:** company finance hub/tesorería/GL solo `OWNER|ADMIN|FINANCE|VIEWER` vía `hasCompanyFinanceRole` + helpers `canViewCompany*`.
