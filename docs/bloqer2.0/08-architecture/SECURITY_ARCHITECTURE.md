...
- **Self-signup (unirse a tenant existente):** no hay registro público arbitrario; unirse al tenant es vía invitación **PENDING** no vencida + email coincidente ([D-064](../00-product/DECISION_LOG.md#d-064--invitación-por-email-al-tenant-q-015)).
- **Credentials email/password (ADR-Auth-Credentials-01):** registro sin sesión hasta `emailVerified`; tokens de verify/reset en `verification_tokens` con purpose prefix + sha256; **verify consume el token solo por POST** (evita prefetch de clientes de correo); takeover de stub no verificado al login Google (**borra `passwordHash`** del stub y setea `passwordUpdatedAt`); anti-enumeración en register/reset; `passwordUpdatedAt` + check Node en `getSession` post-reset.
