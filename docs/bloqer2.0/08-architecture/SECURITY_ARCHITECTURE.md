...
- **Email:** envío vía Resend **opcional**; la invitación se crea igual. Si el correo **no se despachó** (credenciales ausentes, `getPublicAppBaseUrl()` vacío o inválido, o error de proveedor), el enlace con token se muestra una sola vez al admin vía cookie httpOnly de corta duración con **path** `/configuracion/equipo/invitaciones`.
- **Self-signup:** no hay registro público arbitrario; unirse al tenant es vía invitación **PENDING** no vencida + email coincidente.
