...
- **Header (in-app “push” UX, [D-054] / [D-087]):** `NotificationBell` dropdown — last **5** non-archived notifications, badge **only when `unreadCount > 0`**, footer “Ver todas” → `/notificaciones`. Client polls `GET /api/notifications/bell` every **30s** while the tab is visible (pauses when hidden); also refreshes on open. Initial unread seed from SSR in `app/(app)/layout.tsx` (try/catch → `0`). **No** company or project sidebar item.
- **Page:** `/notificaciones` — filters, **pagination 20**/page (`?page=`), mark read / mark unread / mark all read / archive, optional `actionUrl` link (defense-in-depth filter on render). No search in this phase. Access is via header bell (“Ver todas”); Field **Más** keeps a link. **no** Config sidebar item. Server actions revalidate `/notificaciones` and `/` layout so the SSR badge stays consistent after mutations.
