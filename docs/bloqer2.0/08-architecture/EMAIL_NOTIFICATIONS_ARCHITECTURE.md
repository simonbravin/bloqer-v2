...
- **Templates:** `notification-email.ts`, `operational-alert-email.ts`, `auth-email.ts` — mismo layout transaccional (encabezado Bloqer · organización, título, cuerpo, tabla de contexto, ítems, CTA, footer). El **Subject** de notificaciones, invitaciones y reportes se prefija con el tenant (`[Indari] …`). Auth (verificar email / reset) no lleva tenant porque el destinatario puede no tener workspace aún. Invitación: organización, quién invitó y roles. Reportes: organización, proyecto si aplica, nombre de archivo. El servicio de notificación resuelve contexto desde `tenantId` / `companyId` / `projectId` / entidad vinculada (SC, OC, facturas, certificación, parte de obra, recepción, documento, stock) al momento del send.
- **Service (`notification-email.service.ts`):** `sendNotificationEmail`, `sendOperationalAlertEmail` (solo tipos operativos Phase 8B). Carga `Notification` por `tenantId`, valida destinatario con membresía ACTIVE, envía al **email del usuario** (`User.email`). El **Subject** del correo se deriva del título de la notificación, prefijado con el nombre del tenant (`[Indari] Nueva solicitud de compra`) y saneado de saltos de línea (mitigación básica de header injection). No muta la notificación, no marca leída, no crea filas nuevas. AuthZ: **recipient** o **OWNER** / **ADMIN** (`canRunOperationalAlerts`).
