...
4. El sistema ejecuta `syncScheduleProgressFromJobsiteLog` dentro de la misma transacción:
   ...
   - Actualiza `progressPct` y, si el acumulado llega a 100 %, completa la tarea (`PLANNED` o `IN_PROGRESS` → `COMPLETED`) ([D-045]).
...
- Acumulado físico > 100 % en un WBS: la sync **clamp** a 100 % y puede completar la tarea (Q-005b / [D-045]). Ítems ya COMPLETED no se vuelven a pisar.
...
- Botón **Completar** / recepción: `PLANNED → COMPLETED` solo para hitos ([D-104]). Avance real al 100 % (libro o % manual) también completa `TASK` desde `PLANNED` o `IN_PROGRESS` ([D-045]).
...
- [ ] Aprobar un parte que deja una tarea en **100 % Real** (desde Planificado) → estado **Completado**, no figura en Solo atrasados ([D-045]).
