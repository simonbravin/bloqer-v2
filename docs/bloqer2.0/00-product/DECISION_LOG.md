...
- **Decisión:**
  ...
  2. **Persistir `costType`** en `PurchaseRequestLine`, `PurchaseOrderLine` y `SupplierInvoiceLine`. Resolución (ver `resolveLineCostType`): (a) tipo explícito del usuario; (b) categoría del insumo APU si eligió uno; (c) **categoría única del APU de la partida** cuando solo eligió la partida (`computeDominantCostTypeFromApuLines`, `loadWbsDominantCostTypes`) — solo si el APU tiene **una** categoría con costo > 0 (p. ej. baño químico = EQP); si el APU es mixto → null y fallback MATERIAL; (d) fallback MATERIAL. Consumo stock siempre MATERIAL; subcontrato/cert siempre SUBCONTRACT.
  3. **UI OC / factura:** el select "Tipo de costo" pre-selecciona la categoría única al elegir la partida (baño químico → EQP). Partida mixta → MATERIAL + hint “varios tipos, elegí el correcto”. Si el usuario cambia el tipo a mano, el cambio de partida ya **no** lo pisa (`manualCostTypeKeys`).
...
- **Amend (2026-09-07):** el auto-tipado **ya no usa umbral ≥ 60%**. Partidas mixtas (p. ej. Replanteo 79% LAB + 21% MAT) defaultan a MATERIAL en texto libre; tipar LAB/EQP requiere elegir insumo APU, venir desde Pedir del tablero, o elegir el tipo a mano. Evita que clavos/tablas contaminen Mano de obra / EDT filtro LAB. Tableros LAB/EQP además ignoran SC `lineType=MATERIAL` sin APU tipadas LAB/EQP (defensa de datos legacy).
