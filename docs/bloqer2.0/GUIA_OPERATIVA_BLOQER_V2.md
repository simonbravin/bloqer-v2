...
2. **A quién se le paga** (proveedor o empleado si no hay OC), fechas, líneas (cada línea con **partida EDT obligatoria**, D-055; **Tipo de costo**; **insumo APU** opcional — se limpia si cambiás la partida; **Desc. %** opcional antes de IVA; **Descuento general %** para copiar el mismo % a todas), **Percepción IIBB** a nivel documento (alícuota editable, default **3%** sobre el neto; el total = subtotal + IVA + percepción [D-112]), **adjunto** opcional (foto/PDF del comprobante).
