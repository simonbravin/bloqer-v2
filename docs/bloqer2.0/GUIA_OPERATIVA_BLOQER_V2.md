...
4. Guardar `DRAFT` → **Enviar** → `SUBMITTED` (snapshot de costo presupuestario / cantidad por partida EDT). En el **detalle** de la SC, debajo del título, hay una **pista de proceso** compacta (Borrador → Enviada → Cotizando → Adjudicada → Completada) que se actualiza con el estado; el badge de estado se mantiene.
...
10. Notificaciones: envío a compras + recordatorio SLA si demora. Quienes pueden cotizar también ven la SC en **Pendientes** hasta adjudicar el resto / cubrir ítems ([D-094]). El email de nueva solicitud muestra organización, proyecto, solicitante e ítems; el asunto es `[organización] Nueva solicitud · SC-003`.
