...
4. Guardar `DRAFT` → **Enviar** → `SUBMITTED` (snapshot de costo presupuestario / cantidad por partida EDT). En el **detalle** de la SC, debajo del título, hay una **pista de proceso** compacta (Borrador → Enviada → Cotizando → Elegida → Completada) que se actualiza con el estado; el badge de estado se mantiene.
