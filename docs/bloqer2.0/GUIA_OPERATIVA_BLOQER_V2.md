...
- **Extracto = detalle de cuenta:** abrí **Tesorería → Cuentas → [cuenta]**. Ahí ves el ledger (Confirmado / Conciliado), export CSV/PDF y atajos a transferir / conciliar / ajuste manual. Columnas compactas: **Clase** (badge), estado, factura interna (`FP-#####` / `FAC-#####`) y descripción. Tipo y origen quedan en el **detalle** al clic en la descripción (también importe, proyecto, contraparte).
...
3. Adjuntar fotos y observaciones. En **Adjuntos**, la columna **Acciones** es una sola fila de iconos: ojo (**Ver**), flecha (**Descargar**), caja (**Archivar**) y tacho (**Eliminar**). Archivar y eliminar piden confirmación. En fotos **jpeg/png/webp**, clic en el **nombre del archivo**, en **Ver** o en la miniatura (celular) abre un **visor a pantalla completa** con las demás fotos del mismo listado (flechas / teclado). Los **PDF** siguen abriéndose en una pestaña nueva o desde el detalle; HEIC u otros tipos se descargan o se abren según el navegador.
...
3. **Agregar documento** (solo en Planos, General o sus subcarpetas, o en la vista **Todos**) sube el archivo a la biblioteca de la obra (hasta **50 MB**; PDF, imágenes, Word, Excel, CSV, texto, **DWG/DXF**). No hay preview in-app de CAD: Ver/Descargar baja el archivo. Al subir, elegí el **tipo**: Contrato, Plano, Permiso, Técnico, Foto / evidencia, Factura, Remito, Certificado, Informe, Evidencia obra, **Presupuesto**, **Cotización** u Otro. El listado se puede filtrar por el mismo tipo. En carpetas operativas (Libro, OC, …) no hay botón de subir: se completan desde la ficha de la entidad.
4. En la tabla, la columna **Acciones** no muestra texto: el icono de ojo **abre** el archivo. En fotos **jpeg/png/webp**, clic en el **nombre** o en **Ver** abre un **visor a pantalla completa** (si hay varias en el listado, flechas / teclado para pasar); los **PDF** (y HEIC según el navegador) se abren en una pestaña nueva o desde el detalle. El icono de descarga **baja** el archivo. En el detalle, clic en la preview también amplía la foto.
...
4. Guardar `DRAFT` → **Enviar** → `SUBMITTED` (snapshot de costo presupuestario / cantidad por partida EDT). En el **detalle** de la SC, debajo del título, hay una **pista de proceso** compacta (Borrador → Enviada → Cotizando → Adjudicada → Completada) que se actualiza con el estado; el badge de estado se mantiene. Mientras esté en **Borrador**, **Editar** abre el formulario (`…/editar`) para corregir cantidades, líneas, fecha y notas.
5. **Devolver a borrador** ([BR-PUR-025]): si la SC está **Enviada / Cotizando** (`SUBMITTED`) y **todavía no hay cotizaciones activas** (`DRAFT`/`RECEIVED`/`SELECTED`; rechazadas/supersedidas no cuentan) **ni OC**, quien puede editar solicitudes puede **Devolver a borrador** con **motivo obligatorio**. Vuelve a `DRAFT`, limpia el snapshot y se puede editar y reenviar. **Con cotizaciones activas u OC no se puede** devolver: hay que anular OC / gestionar cotizaciones, o anular y recrear.
6. Cargar **Cotizaciones**: elegí proveedor (buscador: razón social o nombre fantasía), **precio unit.**, **Desc. %** (opcional, antes de IVA), **alícuota IVA** y, si el proveedor te pasó el **precio final con IVA** (Factura B), marcá **El precio unitario incluye IVA**. El formulario muestra neto, IVA, **Percepción IIBB** (alícuota editable, default 3% sobre el neto [D-112]) y total del documento antes de guardar. También **plazo de entrega en días** + validez. En el listado se ve el total y, debajo del proveedor, cada línea con precio neto y **Desc. %**. Mientras la SC esté enviada y la cotización **Recibida** (sin usar en una OC activa), podés **Editar** o **Eliminar** una cotización cargada de más. **Cotizaciones ya guardadas no se recalculan**: quedan con los importes que tienen; el checkbox aplica al cargar/editar de acá en más. Cumplir mínimo de cotizaciones de `/configuracion/politicas`. El umbral que obliga SC+cotizaciones vs OC directa lo setea cada empresa en políticas de compras (no es un monto fijo del producto).
7. **Adjudicar** ([BR-PUR-024]): cada **ítem completo** de la SC va a un solo proveedor (no se parten cantidades). En **desktop**: **Adjudicar por ítem** (matriz línea × cotización; por celda: **P. unit.** neto y **Total** de línea con desc./IVA — mismo importe que copia la OC; cantidades a 2 dp) → **Generar órdenes** crea una o más OC en borrador en un solo paso. Atajo por cotización (desktop y celular): **Adjudicar libres → OC** adjudica los ítems **todavía libres** de esa cotización. En celular no está la matriz: para repartir ítems entre proveedores usá escritorio. Cobertura parcial deja la SC en **Enviada** (en Pendientes: **Adjudicar resto**); al cubrir el 100 % pasa a **Adjudicada**. La SC **Completada** recién cuando todas las OC de cobertura están confirmadas. **Anular** la SC solo aparece si no hay OC activas; si ya hay cobertura, anulá primero las OC. Si anulás una OC de cobertura, esos ítems vuelven libres: podés **editar/eliminar** la cotización (si quedó Recibida y sin OC activa) y readjudicar.
8. En el **listado desktop** de solicitudes: **Código**, **Estado**, **Descripción** (primera línea + “+N más” si hay varias), **EDT** (partida o “Múltiple”), **Monto est.** (Σ cant. × ref. presup. **de cada insumo APU ligado** en la SC, o **Σ de las OC activas** en ARS / cotización si hay una sola; badge **Presup.** / **Cotización** / **Órdenes**; con varias OC el proveedor puede verse como **Múltiple**), **Proveedor** (o **Múltiple** si hay varias OC), **Necesaria para**. Columnas **Código** y **Monto est.** se ordenan con el ícono ↑↓. El **solicitante** y fechas de envío/creación se ven en el **detalle** de la SC.
9. **Buscador + filtro de estado** ([D-096]): arriba del listado hay un buscador (código, descripción, EDT, proveedor, solicitante) y botones **Activas / Borrador / Enviada / Adjudicada / Completada / Anulada** con contador por estado. **Activas** (por defecto) oculta las anuladas; el chip **Anulada** las muestra. Los deep-links históricos `?status=SUBMITTED` (por ejemplo desde el email de nueva SC) siguen abriendo el filtro pre-seleccionado; después, quitarlo o cambiarlo se hace desde los mismos botones.
10. **Fecha requerida vencida** ([BR-PUR-019] · [D-097]): mientras la SC esté `SUBMITTED` o `QUOTE_SELECTED`, si `neededByDate` pasó, en el listado y en **Pendientes** aparece el badge rojo **Vencida N d** junto a **Necesaria para**. La alerta sigue hasta que **todos** los ítems estén adjudicados y sus OC estén confirmadas (no alcanza con confirmar una sola OC parcial). Dedup 7 días; colchón y toggle en **Configuración → Políticas de compras**.
11. Notificaciones: envío a compras + recordatorio SLA si demora. Quienes pueden cotizar también ven la SC en **Pendientes** hasta adjudicar el resto / cubrir ítems ([D-094]). El email de nueva solicitud muestra organización, proyecto, solicitante e ítems; el asunto es `[organización] Nueva solicitud · SC-003`. Al **devolver a borrador** se notifica a la misma audiencia del envío con el motivo.
...
En el **detalle** de la OC, debajo del título, hay una **pista de proceso** (Borrador → Aprobar → Confirmar → Recibir → Facturar → Pagar) derivada del estado y de recepción/factura/CxP; el badge de estado se mantiene. Atajos (autorizar y comprometer / aprobar y confirmar) marcan los pasos salteados como hechos. Si ya hay factura emitida pero todavía queda monto por facturar, el paso actual sigue en **Facturar** (no salta a Pagar).

1. **Nueva OC** desde listado/tablero (`?create=1` en escritorio; en celular `/proyectos/[id]/ordenes-compra/nueva`) o desde SC adjudicada (una o más OC por proveedor). Proveedor: buscador por razón social o nombre fantasía. **OC generada desde SC:** en **Editar** no se cambia el proveedor ni se agregan/quitan ítems (quedan fijados por la adjudicación); sí se pueden ajustar cantidades/precios dentro del techo de la cotización. Cada línea: **partida hoja**, **Tipo de costo** (etiqueta **Sugerido desde APU**), luego **unidad / cantidad / precio unitario / Ref. presupuesto**, y debajo **Desc. % · IVA · total**. En el pie del documento: **Percepción IIBB** (alícuota editable, default 3% sobre el neto; se copia desde la cotización si viene de SC [D-112]). **Descuento general %** + **Aplicar a todas** copia el mismo % a cada línea (hay que ingresar un número; 0 limpia todas). **Ref. presupuesto** es el insumo MATERIAL del APU (elegí **Insumo APU** para tomar su $/u), o **costo dir. /u** de la partida si no hay materiales y la unidad es comparable. Si la partida está en **global (`gl`)** y la línea en un/kg/m2, Ref. muestra — (el total de la partida no es un precio unitario). El saldo de partida es aviso, no bloqueo (ver [D-095]).
2. **Varias OC desde una SC** ([BR-PUR-024]): recepción, factura proveedor y pago son **por cada OC** (parcial o total independientes). Anular una OC libera solo sus ítems en la SC; las demás OC siguen.
3. **Listado OC — buscador + filtro de estado** ([D-096]): arriba del listado hay un buscador (código, proveedor, aprobador) y botones **Activas / Borrador / Pend. aprobación / Aprobada / Confirmada / Recep. parcial / Recibida / Anulada** con contador por estado. **Activas** (por defecto) oculta las anuladas; el chip **Anulada** las muestra. En la tabla desktop, **Código** y **Total** se ordenan con el ícono ↑↓. Los deep-links históricos `?status=` siguen funcionando como estado inicial (por ejemplo desde **Pendientes** o el tablero).
4. **Enviar a aprobación** → `SUBMITTED` (aprobadores: Pendientes + campana). Si quien envía puede aprobar y la OC no es de alto nivel, puede auto-aprobarse; con política [D-107] ON esa auto-aprobación deja la OC **Confirmada**.
5. Aprobador: **Aprobar** → `APPROVED` (control interno; badge outline = todavía no reserva $), o **Devolver a borrador** con **motivo obligatorio**. Con política **Al aprobar, confirmar** ([D-107]) ON y OC no alto nivel, el botón dice **Aprobar y comprometer** y deja `CONFIRMED` (Comprometido). Quien puede confirmar la ve en **Pendientes** (etapa Confirmar) solo si quedó Aprobada; campana de “OC aprobada” llega a origen + quien confirma (si auto-confirmó, llega aviso de confirmada, no “pendiente confirmar”).
6. **Confirmar al proveedor** → `CONFIRMED` = **comprometido** en EDT y costos (badge primary = ya reserva $).
7. **Autorizar y comprometer** ([D-105]/[D-106]): en **Configuración → Políticas → Compras → Atajos operativos**, OWNER/ADMIN puede prender **Un paso: autorizar y comprometer** (apagado por defecto). Con ON: OC bajo umbral → PM/Compras; **alto nivel** → solo OWNER/ADMIN. Un acto autoriza y reserva $ (Confirmada). Si en Pend. aprobación también aplica [D-107], Autorizar es el primario y Aprobar queda secundario (outline). Segregación de auto-aprobación sigue valiendo.
8. **Al aprobar, confirmar** ([D-107]): política en **Atajos operativos** (apagada por defecto). Con ON, aprobar una OC **no** de alto nivel la deja Confirmada = Comprometido (sin card «Confirmar» en Pendientes). Alto nivel no auto-confirma.
9. **Al recibir, borrador de factura** ([D-108]): política en **Atajos operativos** (apagada por defecto). Al confirmar recepción se crea un **borrador** de factura (no CxP). Si falla, la recepción igual queda confirmada y se muestra aviso en pantalla. Emitir sigue creando Devengado + CxP.
10. **Registrar recepción** (parcial o total). Quien puede recibir (Compras / Depósito / PM) la ve en **Pendientes** con botón **Recibir** (abre `…/recepciones/nueva`, o el borrador existente). La campana de confirmación avisa “Ya se puede registrar la recepción” con CTA **Registrar recepción** a la misma ficha.
11. **Entrega prevista vencida sin recepción** ([BR-PUR-018] · [D-097]): mientras la OC esté `CONFIRMED` o `PARTIALLY_RECEIVED` con `expectedDeliveryDate` pasada, en el listado y en **Pendientes** aparece el badge rojo **Vencida N d** junto a **Entrega prevista**. Se envía notificación diaria a quien puede recepcionar con deep-link al form (CTA **Registrar recepción**), CC OWNER/ADMIN, dedup 7 días. Ajustable con `deliveryOverdueGraceDays` y toggle `deliveryAlertsEnabled` por empresa.
12. Con cantidades recibidas: panel **Facturación de la OC** → **Registrar factura** / **Actualizar borrador** (refresca líneas si hay más recepción) o **Completar factura** si ya hay borrador. Pendientes dice lo mismo.
13. **OC recibida sin factura registrada** ([BR-PUR-020] · [D-097]): apenas hay recepción confirmada y la OC no tiene factura de proveedor `ISSUED`, aparece en **Pendientes** (grupo Compras → Facturar) para quien tiene `EDIT AP`, con CTA **Registrar factura** (abre la OC con el panel destacado vía `?siguiente=facturar`) o **Completar factura** si hay borrador. La **alerta/campana diaria** espera `receiptToInvoiceSlaDays` (default **5**) desde la primera recepción confirmada, CC OWNER/ADMIN, dedup 7 días. Toggle `receiptToInvoiceAlertsEnabled` por empresa. Es la señal que evita que la CxP nunca se genere y que el pago quede en el aire.
14. Desvíos de precio vs referencia: la ficha **siempre** muestra **Ref. presup.** y **Desvío** por línea (también en celular). Si el PU **supera** el referencial (umbrales de políticas), pide **Justificación desvío** en **Editar**; el aviso lista las líneas. Comprar por debajo no exige nota (el % sale en verde). Partida en **global** vs línea física: Ref. = — con texto «no es un $/u» y, si hay un insumo APU en la misma unidad, lo sugiere; Desvío = «Sin $/u comparable»; no bloquea. En **Editar** cada línea muestra PU · Ref · Desvío. Sin referencial comparable (APU y costo dir. /u en cero, unidad física) sí pide justificación. El caminito: [`guides/assets/flujos/mapa-flujo-desvio-oc-si-no.png`](./guides/assets/flujos/mapa-flujo-desvio-oc-si-no.png).

![Bloqer — Desvío de OC: si esto, entonces aquello](./guides/assets/flujos/mapa-flujo-desvio-oc-si-no.png)
15. **OC directa** (sin SC): solo si la política de compras lo habilita; umbrales altos pueden exigir motivo de emergencia (`OWNER`/`ADMIN`).
...
1. Desde **Pendientes** (botón **Recibir**), la **campana** (CTA **Registrar recepción**), la OC (**Registrar recepción** / **Continuar recepción**) o Compras → **Recepciones**. Todos los caminos usan la **misma ficha**.
2. Si ya hay una recepción en **Borrador** para esa OC, el sistema **abre ese borrador** (no crea otro). En borrador podés editar **fecha**, cantidades, notas y depósito.
3. **Guardar borrador** deja la recepción pendiente; **Confirmar recepción** cierra el ingreso (stock si hay depósito/producto). Cantidades pendientes se muestran a **2 decimales** y ya descuentan lo reservado en borradores.
...
1. **Nuevo subcontrato** (`/proyectos/[id]/subcontratos/nuevo`): subcontratista del directorio (rol **Subcontratista**; buscador: razón social o nombre fantasía), alcance e imputación a partidas con categoría **SUB** en APU cuando corresponda. **Cada línea del subcontrato debe tener partida EDT** antes de aprobar certificaciones (requerido para emitir la factura AP, [D-055]). En el alta, **Adjuntos (opcional)** usa la misma zona que facturas: arrastrar, **Seleccionar archivo** o **Tomar foto** (máx. 8 archivos y 45 MB en total). Si un adjunto falla, el subcontrato igual se crea y se reintenta desde el detalle → **Adjuntos**. En **Editar** no se suben archivos: usá el panel del detalle.
...
- **Facturas emitidas** (`/proyectos/[id]/facturas`, estados Borrador / Emitida / Anulada): una vez emitidas son inmutables; solo se pueden **anular**. Detalle: **Emitir** desde borrador; panel de **adjuntos** del comprobante. Al crear, el cliente se busca por razón social o nombre fantasía. En la línea: **Desc. %** opcional (antes de IVA; el precio unitario es de lista). En el pie: **Percepción IIBB** (alícuota editable, default 3% sobre el neto [D-112]). En el listado: buscador por código (`FAC-` / `NC-` / `ND-`) o cliente, y filtro **Tipo de documento**. El campo fiscal de letra es **Comprobante** (A/B/C/E), no “tipo de factura”.
...
1. Orden del formulario: **A quién se le paga** → **Comprobante** (letra A/B/C/E) → fechas de emisión/vencimiento → **Imputación de costo** (toggle **Contra orden de compra** / **Costo directo**, con badge **Compra comprometida** o **Costo directo de obra**) → checkbox **El precio unitario incluye IVA**.
2. Modo Contra OC: picker de OC (**confirmadas, parcialmente recibidas o recibidas** del mismo proveedor) + “Traer líneas” (baja el comprometido abierto; copia partida EDT, tipo de costo e **insumo APU** de la OC si había, [D-110]). Costo directo: sin OC; imputá partida y tipo de costo (no reduce comprometido). Desde Operación → **Mano de obra** / **Equipos** → **Factura**, el alta llega tipada LAB/EQP con vínculo APU prellenado. Borradores nacidos desde la OC (**Completar factura**) quedan fijos en Contra OC (proveedor y OC no se cambian).
3. Líneas (cada una con **partida EDT obligatoria**, D-055; **Tipo de costo**; **insumo APU** opcional — se limpia si cambiás la partida; **P. unit.** / **P. unit. (c/IVA)**; **Desc. %** opcional antes de IVA; **Descuento general %** para copiar el mismo % a todas), **Percepción IIBB** a nivel documento (alícuota editable, default **3%** sobre el neto; el total = subtotal + IVA + percepción [D-112]), **adjunto** opcional (foto/PDF del comprobante).
4. Desde OC / recepción: botón **Registrar factura** del panel Facturación abre en modo Contra OC y copia la partida EDT (y APU) de cada línea.
5. Sin más: **Crear factura** → queda en **borrador** → luego **Emitir** en el detalle (crea CxP + **asiento DRAFT** en contabilidad, ver §15).
6. Con permiso **EDIT tesorería** y módulo Tesorería activo: checkbox **Emitir y pagar ahora (egreso de caja)** → cuenta de pago + fecha → **Emitir y pagar**. Crea factura emitida + CxP + pago + egreso en un paso. Si no hay fondos suficientes, **bloquea**.
...
- **Tipo en OC / factura de obra:** cada línea tiene **Tipo de costo**. **Auto-tipado al elegir partida ([D-099]):** si el APU de la partida tiene **una sola** categoría con costo (p. ej. baño químico = EQP), la línea se pre-tipa con esa. Si el APU es **mixto** (aunque una categoría sea mayoría), queda **Materiales** con hint “varios tipos, elegí el correcto” — evitá que un pedido de clavos en Replanteo se tipifique como Mano de obra. Si elegís un insumo APU específico, prevalece su categoría. Desde Pedir en Materiales / Mano de obra / Equipos, el tipado viene del tablero. Si cambiaste el tipo a mano, no se pisa al mover la partida.
...
| **Impuestos / retenciones** | IVA por línea + **Percepción IIBB** a nivel documento (alícuota editable, default 3% sobre neto [D-112]); otras retenciones manuales, sin módulo dedicado. |
