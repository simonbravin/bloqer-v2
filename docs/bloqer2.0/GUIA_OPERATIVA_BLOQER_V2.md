...
1. **Nuevo subcontrato** (`/proyectos/[id]/subcontratos/nuevo`): subcontratista del directorio (rol **Subcontratista**; buscador: razón social o nombre fantasía), alcance e imputación a partidas con categoría **SUB** en APU cuando corresponda. **Cada línea del subcontrato debe tener partida EDT** antes de aprobar certificaciones (requerido para emitir la factura AP, [D-055]). En el alta podés **Adjuntar** el contrato u otros archivos (PDF, imagen, Word, Excel, CSV, texto; máx. 8 archivos y 45 MB en total). Si un adjunto falla, el subcontrato igual se crea y se reintenta desde el detalle → **Adjuntos**. En **Editar** no se suben archivos: usá el panel del detalle.
