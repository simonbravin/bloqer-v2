...
- **Excepción ([D-088]):** si `Tenant.allowApprovedBudgetEconomicEdits` **y** `Project.allowApprovedBudgetEconomicEdits` están ambos `true`, se permite **edición económica completa** del presupuesto `APPROVED` de esa obra (WBS, cantidades, PU, APU, `BudgetSettings`). Default de ambos flags: `false`. **`CLOSED` nunca** entra por esta excepción ([BR-BUD-002]). Mutaciones bajo override quedan auditadas (`approvedEditOverride`). Encender/apagar flags: solo OWNER/ADMIN; editar con override activo: `EDIT BUDGETS`.
