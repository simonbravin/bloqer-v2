...
- **Compras ([D-050], [BR-PUR-015], [D-094], [D-097], [D-105]):** in-app + email en cambios de estado de SC/OC; recordatorio por antigüedad con escalamiento a OWNER/ADMIN. El cuerpo in-app y el email incluyen organización, proyecto y solicitante; el Subject del mail va prefijado con el tenant (`[Indari] …`). Fallo de email = best-effort (no aborta la mutación). Audiencia ampliada de follow-through: `PURCHASE_ORDER_APPROVED` también a quien puede confirmar al proveedor; `PURCHASE_ORDER_CONFIRMED` a quien puede registrar recepción (Depósito / Compras / PM). **Atajo Autorizar y comprometer ([D-105]):** no emitir `PURCHASE_ORDER_APPROVED`; solo `PURCHASE_ORDER_CONFIRMED`. La cola operativa (SC a cotizar, OC a confirmar, OC a recibir, OC recibida sin factura) también aparece en **Pendientes** ([D-094], [D-097]); CxP **Listo para pagar** sigue solo en campana.
  - **Alertas de vencimiento** ([D-097]) — corren en el mismo cron `operational-alerts`, dedup 7 días por tipo+entidad+recipient, toggles y colchones por empresa en `CompanyProcurementSettings`:
    - `PURCHASE_ORDER_DELIVERY_OVERDUE` — OC `CONFIRMED/PARTIALLY_RECEIVED` con `expectedDeliveryDate` vencida. Audiencia: `EDIT PROCUREMENT|PURCHASE_ORDERS|INVENTORY` + CC OWNER/ADMIN. Deep-link al form de recepción. [BR-PUR-018].
    - `PURCHASE_REQUEST_NEEDED_BY_OVERDUE` — SC `SUBMITTED/QUOTE_SELECTED` con `neededByDate` vencida y sin OC confirmada. Audiencia: aprobadores SC/OC + CC OWNER/ADMIN. [BR-PUR-019].
    - `PURCHASE_ORDER_RECEIVED_WITHOUT_INVOICE` — OC `PARTIALLY_RECEIVED/RECEIVED` con primera recepción confirmada hace ≥ `receiptToInvoiceSlaDays` (default 5) sin `SupplierInvoice` en `ISSUED`. Audiencia: `EDIT|APPROVE AP` + CC OWNER/ADMIN. [BR-PUR-020].
