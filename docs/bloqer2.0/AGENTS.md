...
5. **El dinero nunca se guarda como `float`**. Siempre `decimal` con precisión de negocio definida en [`03-finance/MONEY_MODEL.md`](./03-finance/MONEY_MODEL.md) y [D-053](./00-product/DECISION_LOG.md): montos operativos y `amount_ars` a **2** dp half-up; FX a **6**; qty/% a **4**. Usar el kernel `@bloqer/utils` (`roundMoney` / `serializeMoney`); DTOs money sin `.toString()` crudo; UI con `formatMoneyAmount`.
