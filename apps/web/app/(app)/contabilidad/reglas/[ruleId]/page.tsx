  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="rounded-lg border bg-card p-6 space-y-2 text-sm">
        ...
        <p><span className="text-muted-foreground">Prioridad:</span> {rule.priority}</p>
        {rule.description && (
          <p><span className="text-muted-foreground">Descripción:</span> {rule.description}</p>
        )}
        <p><span className="text-muted-foreground">Debe:</span>{" "}
          <span className="font-mono">{rule.debitAccountCode}</span>
        </p>
        <p><span className="text-muted-foreground">Haber:</span>{" "}
      ...
      <div className="rounded-lg border bg-muted/30 p-6 text-sm text-muted-foreground space-y-2">
        ...
        <p>
          Las reglas activas se usan al generar borradores desde cobranzas, pagos, tesorería o consumos de inventario (fase 11C).
          Nada se contabiliza ni se postea en automático: siempre revisá el asiento en Contabilidad.
        </p>
      </div>
