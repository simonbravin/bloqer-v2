    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" asChild>
            <Link href={`/contabilidad/asientos${q}`}>← Volver</Link>
          </Button>
        ...
        {canEdit && entry.status === "DRAFT" && (
          <Button variant="outline" asChild>
            <Link href={`/contabilidad/asientos/${journalEntryId}/editar${q}`}>Editar borrador</Link>
          </Button>
        )}
      </div>
      ...
      <div className="rounded-lg border bg-card p-6 space-y-2 text-sm">
        <p><span className="text-muted-foreground">Fecha:</span>{" "}
          <span className="font-mono">{entry.entryDate.toISOString().slice(0, 10)}</span>
        </p>
        <p><span className="text-muted-foreground">Descripción:</span> {entry.description}</p>
        {entry.reference && (
          <p><span className="text-muted-foreground">Referencia:</span> {entry.reference}</p>
        )}
        <p><span className="text-muted-foreground">Origen:</span> {entry.sourceType}</p>
      </div>
            ...
            <TableBody>
              {entry.lines.map((l) => (
                <TableRow key={l.id}>
                  ...
                  <TableCell className="max-w-[200px] truncate text-sm text-muted-foreground">{l.description ?? "—"}</TableCell>
                  <TableCell className="text-right font-mono text-sm">{l.debit !== "0" ? l.debit : "—"}</TableCell>
                  <TableCell className="text-right font-mono text-sm">{l.credit !== "0" ? l.credit : "—"}</TableCell>
