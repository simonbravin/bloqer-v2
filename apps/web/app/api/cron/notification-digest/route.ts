...
import {
  runNotificationDigestForAllActiveTenants,
  runNotificationDigestForTenant,
...
async function handleNotificationDigestCron(req: NextRequest): Promise<NextResponse> {
  ...
  if (tenantIdRaw !== null && tenantIdRaw !== "") {
    ...
    const result = await runNotificationDigestForTenant(parsed.data);
    return NextResponse.json(result);
  }

  const result = await runNotificationDigestForAllActiveTenants();
  return NextResponse.json(result);
}

export async function POST(req: NextRequest): Promise<NextResponse> {
  return handleNotificationDigestCron(req);
}
