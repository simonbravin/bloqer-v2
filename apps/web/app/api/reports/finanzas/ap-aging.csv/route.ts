...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (fmt === "json") {
      const data = await getPayableAgingReport(filters, auth.ctx);
      return NextResponse.json(data);
    }
    if (fmt === "pdf") {
      const { buffer, filename } = await exportPayableAgingPdf(filters, auth.ctx);
      return pdfResponse(buffer, filename);
    }
    if (fmt === "csv") {
      const { content, filename } = await exportPayableAgingCsv(filters, auth.ctx);
