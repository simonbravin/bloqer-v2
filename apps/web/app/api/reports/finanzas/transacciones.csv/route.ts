...
import {
  ...
  FINANCE_TRANSACTIONS_EXPORT_LABELS,
  getAccountMovementReport,
  parseMovementReportFilters,
} from "@bloqer/services";
import { exportTreasuryMovementsPdf } from "@bloqer/report-pdf";
import {
  csvResponse,
  pdfResponse,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (fmt === "pdf") {
      const { buffer, filename } = await exportTreasuryMovementsPdf(
        filters,
        auth.ctx,
        FINANCE_TRANSACTIONS_EXPORT_LABELS,
      );
    ...
    if (fmt === "csv") {
      const { content, filename } = await exportTreasuryMovementsCsv(
        filters,
        auth.ctx,
        FINANCE_TRANSACTIONS_EXPORT_LABELS,
      );
