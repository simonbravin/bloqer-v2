  ...
  return (
    <form onSubmit={apply} className="flex flex-wrap items-end gap-3 rounded-lg border bg-card p-4">
      ...
      <div className="space-y-1">
        ...
        <Input name="dateFrom" type="date" className="h-8 text-xs w-36" defaultValue={params.get("dateFrom") ?? ""} />
      </div>
      <div className="space-y-1">
        <Label className="text-xs">Hasta</Label>
        <Input name="dateTo" type="date" className="h-8 text-xs w-36" defaultValue={params.get("dateTo") ?? ""} />
      </div>
      <div className="space-y-1">
        <Label className="text-xs">Buscar WBS</Label>
        <Input name="wbsSearch" className="h-8 text-xs w-40" placeholder="Código o nombre…" defaultValue={params.get("wbsSearch") ?? ""} />
      </div>
      <Button type="submit" size="sm" className="h-8">Aplicar</Button>
      <Button type="button" variant="ghost" size="sm" className="h-8 text-muted-foreground" onClick={clear}>Limpiar</Button>
    </form>
  );
}
